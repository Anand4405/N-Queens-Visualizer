...
class Queen {
    ...
    isValid = async (board, r, col, n) => {
        ...
        for (let i = 0; i <col; i++) {
            const row = table.firstChild.childNodes[row];
            const column = row.getElementsByTagName("td")[i];

            const value = column.innerHTML;

            if (value == queen) {
                column.style.backgroundColor = "#FB5607"; // red color showing not possible
                currentColumn.innerHTML = "-" // and make that empty
                return false; //  and return false
            }
            column.style.backgroundColor = "rgb(216, 35, 177)";  // this color shows possible
            await q.delay();
        }
        ...
        for (let i = r - 1, j = col - 1; i >= 0 && j >= 0; --i, --j) {
            ...
            column.style.backgroundColor = "rgb(216, 35, 177)";
            await q.delay();
        }

        // Checking the upper right diagonal
        for (let i = r - 1, j = col -1; i <n  && j >= n; i++, j--) {
            const row = table.firstChild.childNodes[i];
            const column = row.getElementsByTagName("td")[j];
    ...
    clearColor = async (board) => {
        for (let j = 0; j < n; ++j) {
            ...
            for (let k = 0; k < n; ++k)
                (j + k) & 1
                    ? (row.getElementsByTagName("td")[k].style.backgroundColor = "rgb(9, 243, 60)")
                    : (row.getElementsByTagName("td")[k].style.backgroundColor = "rgb(243, 239, 9)");
        }
    }
    ...
    solveQueen = async (board, col, n) => {
        ...
        for (let i = 0; i < n; ++i) {
            ...
            await q.clearColor(board);
            if (await q.isValid(board, i, col, n)) {
                await q.delay();
                // console.log("inside:" + board)
                await q.clearColor(board);
                let table = document.getElementById(`table-${this.uuid[board]}`);
                let col = table.firstChild.childNodes[col];
                col.getElementsByTagName("td")[i].innerHTML = queen;

                this.position[board][col] = i;

                if (await q.solveQueen(board, col + 1, n))
                    await q.clearColor(board);
                ...
                row = table.firstChild.childNodes[col];
                row.getElementsByTagName("td")[i].innerHTML = "-";

                delete this.position[`${board}`][`${col}`];
            }
        }
    }
}
    ...
    for (let k = 0; k < array[n]; ++k) {
        ...
        for (let i = 0; i < n; ++i) {
            ...
            for (let j = 0; j < n; ++j) {
                ...
                (i + j) & 1
                    ? (col.style.backgroundColor = "rgb(9, 243, 60)")
                    : (col.style.backgroundColor = "rgb(243, 239, 9)");
                col.innerHTML = "-";
